...
import { Sim } from '../brawl/public/js/sim/sim.js';
import { MAP_LIST } from '../brawl/public/js/sim/maps.js';
import { ROSTER, modsOf } from '../brawl/public/js/sim/roster.js';
import { encodeSnapshot, decodeSnapshot, encodeInputs, decodeInputs, lerpTransforms } from '../brawl/public/js/sim/snapshot.js';
import { qRotate } from '../brawl/public/js/sim/math.js';
...
test('봇 네 명 경기는 끝까지 가고, 우승자는 필요한 승수를 채운다', async () => {
  ...
  for (const t of ['roundStart', 'fight', 'hit', 'ko', 'out', 'roundEnd']) assert.ok(seen.has(t), `${t} 이벤트 없음`);
...
test('스냅샷·입력 이진 형식 왕복', () => {
  ...
  const buf = encodeSnapshot({
    ...
    statuses: [{ slot: 2, flags: 18, hp: 0.5, grab: 'submit', grabProgress: 0.25, victimOf: null, victimProgress: 0, escape: 3.3 }],
  });
  assert.equal(buf.byteLength, 22 + 8 + 2 * 14);
  ...
  assert.ok(Math.abs(s.statuses[0].hp - 0.5) < 0.01);
